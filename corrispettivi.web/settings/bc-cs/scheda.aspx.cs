        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Page.IsPostBack != true)
            {
                if (Convert.ToInt32(Request["id"]) > 0)
                {
                    try
                    {
                        DataTable dt = paramBcCompensiSettimanali.read(Convert.ToInt32(Request["id"]));
                        if (dt.Rows.Count > 0)
                        {
        ...
        protected void tbAction_ButtonClick(object sender, Telerik.Web.UI.RadToolBarEventArgs e)
        {
            try
            {
                ...
                switch (e.Item.Value)
                {
                    ...
                    case "btnSaveClose":
                        {
                            ...
                            result = paramBcCompensiSettimanali.write(id
                                                                , txtDescrizione.Text
                    ...
                    case "btnDelete":
                        {
                            result = paramBcCompensiSettimanali.delete(id, ((authb.UserInfo)Session["UserInfo"]).Username);
