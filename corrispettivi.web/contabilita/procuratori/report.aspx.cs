            ...
            if (e.Item.ItemType == GridItemType.Item || e.Item.ItemType == GridItemType.AlternatingItem || e.Item.ItemType == GridItemType.Footer)
            {
                foreach (TableCell cell in e.Item.Cells)
                {
                    ...
                    if (e.Item.ItemType != GridItemType.Footer && index >= 3 && index <= 14)
                    {
                        ...
                        e.Item.Cells[index].BackColor = corrispettivi.tools.SystemFunction.GetTabulatoStatusColor(Convert.ToInt32(item.GetDataKeyValue(corrispettivi.tools.SystemFunction.GetMonthPrefix(index) + "_CheckFattura").ToString()));
