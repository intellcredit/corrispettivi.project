...
namespace corrispettivi.web.contabilita.procuratori
{
    public partial class periodo : System.Web.UI.Page
    {
        ...
        protected void setAction()
        {
            ...
            tbAction.FindButtonByCommandName("expCsv").NavigateUrl = string.Format(tbAction.FindButtonByCommandName("expCsv").Value, cmbPeriodoRif.SelectedValue, Session["AgenteGroupID"].ToString());
            tbAction.FindButtonByCommandName("expExcel").NavigateUrl = string.Format(tbAction.FindButtonByCommandName("expExcel").Value, cmbPeriodoRif.SelectedValue, Session["AgenteGroupID"].ToString());
            tbAction.FindButtonByCommandName("expPdf").NavigateUrl = string.Format(tbAction.FindButtonByCommandName("expPdf").Value, cmbPeriodoRif.SelectedValue, Session["AgenteGroupID"].ToString());
        }
        ...
        protected void gvInfo_ItemDataBound(object sender, GridItemEventArgs e)
        {
            if (e.Item.ItemType == GridItemType.Item || e.Item.ItemType == GridItemType.AlternatingItem)
            {
                ...
                e.Item.Cells[5].BackColor = corrispettivi.tools.SystemFunction.GetTabulatoStatusColor(Convert.ToInt32(item.GetDataKeyValue("FatturaStatusID")));
            }
            ...
            if (e.Item is GridNestedViewItem)
            {
                ...
                Label idc = (Label)nitem.FindControl("lblCallCenterID");
                Label ida = (Label)nitem.FindControl("lblAgenteGroupID");

                RadToolBar tbRip = (RadToolBar)nitem.FindControl("tbRiepilogo");
                IRadToolBarButton bPdf = tbRip.FindButtonByCommandName("Pdf");
                bPdf.NavigateUrl = string.Format(bPdf.NavigateUrl, p.Text, ida.Text);
                ...
                bRegFatt.NavigateUrl = string.Format(bRegFatt.NavigateUrl, p.Text.Substring(3, 4), p.Text.Substring(0, 2), idc.Text, ida.Text);
